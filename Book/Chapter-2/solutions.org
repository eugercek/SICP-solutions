...
#+end_src
* 2.53
In this question l thought a lot about "Should lists have ='= before them or not'".
At first I thought like when a evaluator see =(1 2 3)= It'll throw error because 1 is not an symbol.
But LISPs are not only consist of evaluator they also have a reader.
Which reads *not evaluates* the S-expression so =(1 2 3)= is an valid list.
Because it's delimited by parens and inside elements are seperated by spaces.
...
* 2.54
This question's answer is long because generally we only look at equals.

In below code we didn't look at if seq is =null= or is it =pair?= etc.
We know that it's seq so code was small.
#+begin_src racket
...
Lets emulate what quote does with lists.
With =equal?= function from previous exercise  we can see that those two lists are equal.

#+begin_src racket :noweb no-export :lang racket
;; '(quote (a b c))
...
(quote (list 'quote 'abracadabra))
(list 'quote (list 'quote 'abracadabra))
#+end_src

It's clearly seen that first element is quote.
Because ='quote= means =(quote quote)= which is a function returns a symbol *without* evaluating it.
* 2.56
#+name: deriv-base
#+begin_src racket :tangle no :eval no
...
#+name: deriv
#+begin_src racket :tangle no :eval no :export no
<<deriv-base>>
...
I had bad experience with =#lang sicp= in quotes sometimes it does weird things.
So I suggest you to use =#lang racket= in these type of exercises.


I assumed that =u= is a number.

#+begin_src racket :lang racket :results drawer
<<deriv-base>>
(define (deriv exp var)
  (cond ((number? exp) 0)
        ((variable? exp) (if (same-variable? exp var) 1 0))
...
'(+ 1 (* 5 (** x 4)))
:end:
* TODO 2.57 :better_high_order:
At first I said like ok sum is easy, them product ...
But with in a second I remembered that we have recursion.

This is really good example for creating abstraction barriers.
Without changing the simple definition of $(ab)' = a'b + ab'$ we can go.

#+name: other-half
#+begin_src racket :lang racket :export no
(define (variable? x) (symbol? x))
...
We can simplify more they both have very same structure.
#+begin_src racket
<<other-half>>
...
** a
:PROPERTIES:
:header-args:racket+: :tangle 2.58-a.rkt
:END:
#+name: only-deriv
#+begin_src racket :tangle no :eval no
(define (deriv exp var)
...
We don't need to deal with more than two operand because it it's impossible :D
...
:END:
=derivative= procedure is same because derivation rules have not changed.
...
=addend= of expression is same but =augend= is different.
#+begin_src racket :tangle no :eval no
(define (sum? exp) (and (pair? exp) (eq? (cadr exp) '+)))
...
We will use lots of =car= =cdr= so, I implemented =nth=
#+name: nth
#+begin_src racket :tangle no :eval no
(define (nth n seq)
  (if (= 0 n)
...
#+begin_src racket :tangle 2.58-b.rkt
<<only-deriv>>
<<deriv-sum>>
<<deriv-product>>
<<base-predicates>>
...
* TODO 2.59
I should make =union-set= more robust
#+name: set-e
#+begin_src racket :tangle no :eval no
...
=element-of-set?=  is same we need to go $\theta(n)$
...
=intersection-of-set=  is same we need to go $\theta(n^2)$
...
=append= is and $\theta(n)$ procedure so =union-set= is $\theta(n)$ too
#+begin_src racket
<<set-e>>
<<set-a>>
<<set-i>>
...
This is $\theta(n)$ (which reduces from 2n).
...
:END:
I get =get= and =put= from http://community.schemewiki.org/?sicp-ex-2.73.
...
** a
Dispatch is checking type of a datum and calling appropiate procedure.
We can't add =number?= to the table because it has no row value, it don't have a operator to select form.
You can't evaluate =(operator exp)= which is =(car exp)= to a number.
Same is true for =variable?=
** b
:PROPERTIES:
:header-args:racket+: :tangle 2.73-b.rkt
:END:
Because we get =(operands exp)= selectors will have one less =cdr=.
#+begin_src racket :tangle 2.73.rkt
...
** c
:PROPERTIES:
:header-args:racket+: :tangle 2.73-c.rkt
:END:
...
#+end_src
** d
We don't need to touch =deriv=.
Just change the put statements in the package.
