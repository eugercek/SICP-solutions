...
Untill now always used map for computing something, but with =(car)=, =(map)= showed it potential for me.

Also notice that =(map cdr seqs)= actually does =(cadr)= because it'll evaluate =(car)= in upper call.

#+begin_quote
But I started to worry about efficency from internet I learned I should don't care about efficency untill book say.
#+end_quote
...
first element's column's nubmer must be same with second element's row's number.
...
=(map)= gives elemetns of list once at time, since elements of list are rows we can what we want.
...
This one has repeating parts we know that =(accumulate)= takes an operator, initla value and a list.
